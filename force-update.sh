...
# Принудительно отбрасываем все локальные изменения и обновляемся
log "Force updating from repository..."
su - "$INSTALL_USER" -c "
    cd $INSTALL_PATH/bot && \
    git fetch origin && \
    ...
    git clean -fd
" || {
    log "Git update failed, restoring from backup..."
...
" || {
    log "Dependency update failed, restoring from backup..."
    rm -rf "$INSTALL_PATH/bot"
    mv "$backup_dir" "$INSTALL_PATH/bot"
    systemctl start "$SERVICE_NAME"
    log "Restore complete. Previous version is running."
